...
function editEmployee(id) {
    ...
    // Fetch and populate site-specific allowances
    populateSiteAllowances(emp.id);
    
    openEmployeeModal('edit');
}

async function populateSiteAllowances(employeeId) {
    const container = document.getElementById('siteAllowancesContainer');
    container.innerHTML = '<div class="loader-small" style="text-align:center; padding:10px;">جاري التحميل...</div>';
    
    try {
        const res = await fetch(`${API_URL}?action=getSiteAllowances&employeeId=${employeeId}`);
        const result = await res.json();
        container.innerHTML = '';
        if (result.success && result.data.length > 0) {
            result.data.forEach(item => {
                addSiteAllowanceRow(item.siteId, item.allowancePrice);
            });
        }
    } catch (e) {
        console.error("Error fetching site allowances:", e);
        container.innerHTML = '<div style="color:var(--danger); font-size:0.8rem;">فشل تحميل البدلات المخصصة</div>';
    }
}

function addSiteAllowanceRow(siteId = '', price = '') {
    const container = document.getElementById('siteAllowancesContainer');
    const row = document.createElement('div');
    row.className = 'site-allowance-item';
    
    let options = '<option value="">اختر الموقع</option>';
    allSites.forEach(site => {
        if (!site.isTemporary) {
            options += `<option value="${site.id}" ${String(site.id) === String(siteId) ? 'selected' : ''}>${site.name}</option>`;
        }
    });
    
    row.innerHTML = `
        <select class="allowance-site-select">${options}</select>
        <input type="number" class="allowance-price-input" placeholder="السعر" value="${price}">
...
function closeEmployeeModal() { 
    ...
    document.getElementById('siteAllowancesContainer').innerHTML = '';
}
...
async function saveEmployee() {
    ...
    try {
        ...
        const result = await res.json();
        if(result.success) {
            // Save site-specific allowances if any
            await saveSiteAllowances(payload.id);
...
async function saveSiteAllowances(employeeId) {
    const container = document.getElementById('siteAllowancesContainer');
    const rows = container.querySelectorAll('.site-allowance-item');
    const allowances = [];
    
    rows.forEach(row => {
        const siteId = row.querySelector('.allowance-site-select').value;
        const price = row.querySelector('.allowance-price-input').value;
        if (siteId && price) {
            allowances.push({ siteId, allowancePrice: price });
        }
    });
    ...
    try {
        ...
        console.error("Error saving site allowances:", e);
